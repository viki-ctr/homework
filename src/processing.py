...
def search_operations(transactions: list[dict], search_bar: str) -> list[dict]:
    """Функцию, принимающая список словарей с данными о банковских операциях и строку поиска, возвращает список словарей с данной строкой"""
    try:
        search_list = []
        for transaction in transactions:
            if re.search(search_bar, transaction["description"], flags=re.IGNORECASE):
...
def count_categories(transactions: list[dict]) -> dict:
    """Функция, принимающая список словарей, возвращает словарь, где ключ - название категорий, значение - кол-во операций"""
    categories = []
    for transaction in transactions:
